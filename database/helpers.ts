...
export async function is_table_exists(database: Database<sqlite3.Database, sqlite3.Statement>, table_name: string) {
	const result = await database.get(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`, [table_name]);
	if (process.env.MODE === "dev") console.log(`[+] Table '${table_name}' status:`, result ? "up" : "down");
	return result !== undefined;
}

export async function init_tables(database: Database<sqlite3.Database, sqlite3.Statement>) {
	if (!(await is_table_exists(database, "types"))) {
		await init_types_table(database);
		if (process.env.MODE === "dev") console.log("[+] Initiated types table successfully");
	}
	if (!(await is_table_exists(database, "honeytokens"))) {
		await init_honeytokens_table(database);
		if (process.env.MODE === "dev") console.log("[+] Initiated honeytokens table successfully");
	}
	if (!(await is_table_exists(database, "alerts"))) {
		await init_alerts_table(database);
		if (process.env.MODE === "dev") console.log("[+] Initiated alerts table successfully");
	}
	if (!(await is_table_exists(database, "whitelist"))) {
		await init_whitelist_table(database);
		if (process.env.MODE === "dev") console.log("[+] Initiated whitelist table successfully");
	}
}

export async function print_table(database: Database<sqlite3.Database, sqlite3.Statement>, table_name: string) {
	try {
...
export async function commit(database: Database<sqlite3.Database, sqlite3.Statement>) {
	await database.run("COMMIT");
}

export async function rollback(database: Database<sqlite3.Database, sqlite3.Statement>) {
