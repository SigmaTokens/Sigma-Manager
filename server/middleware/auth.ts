...
export function auth(mustBeSelf = false): RequestHandler {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!raw) return void res.status(500).json({ action: Constants.ACCESS_DENIED, force: false });

      const tok = verifyBiscuit(raw);

      if (tok === '') return void res.status(500).json({ action: Constants.ACCESS_DENIED, force: true });
      ...
      if (!id || !username) return void res.status(500).json({ action: Constants.ACCESS_DENIED, force: true });
